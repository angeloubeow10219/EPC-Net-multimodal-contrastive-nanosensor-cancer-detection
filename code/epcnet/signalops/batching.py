...
def transform_16_00(values: Tensor, strength: float = 0.20) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
        return values.clone()
    kernel = torch.ones(5, device=values.device, dtype=values.dtype) / 5
    filtered = torch.nn.functional.conv1d(
        values.unsqueeze(1), kernel.view(1, 1, -1), padding=2
    ).squeeze(1)
    return torch.lerp(values, filtered, strength)


def transform_16_01(values: Tensor, strength: float = 0.25) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
...
def transform_16_02(values: Tensor, strength: float = 0.30) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
...
def transform_16_03(values: Tensor, strength: float = 0.35) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 11:
...
def transform_16_04(values: Tensor, strength: float = 0.40) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 3:
    ...
    attenuation = torch.exp(-strength * frequency.square() * 17)
    return torch.fft.irfft(spectrum * attenuation, n=width, dim=-1)


def transform_16_05(values: Tensor, strength: float = 0.10) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
...
def transform_16_06(values: Tensor, strength: float = 0.15) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
        return values.clone()
    kernel = torch.ones(7, device=values.device, dtype=values.dtype) / 7
    filtered = torch.nn.functional.conv1d(
        values.unsqueeze(1), kernel.view(1, 1, -1), padding=3
    ).squeeze(1)
    return torch.lerp(values, filtered, strength)


def transform_16_07(values: Tensor, strength: float = 0.20) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
...
def transform_16_08(values: Tensor, strength: float = 0.25) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 11:
...
def transform_16_09(values: Tensor, strength: float = 0.30) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 3:
...
def transform_16_10(values: Tensor, strength: float = 0.35) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
...
def transform_16_11(values: Tensor, strength: float = 0.40) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
