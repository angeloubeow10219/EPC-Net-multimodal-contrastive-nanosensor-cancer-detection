...
def transform_12_00(values: Tensor, strength: float = 0.35) -> Tensor:
    if values.ndim < 2:
...
def transform_12_01(values: Tensor, strength: float = 0.40) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
...
def transform_12_02(values: Tensor, strength: float = 0.10) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 11:
...
def transform_12_03(values: Tensor, strength: float = 0.15) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 3:
...
def transform_12_04(values: Tensor, strength: float = 0.20) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
    ...
    attenuation = torch.exp(-strength * frequency.square() * 13)
    return torch.fft.irfft(spectrum * attenuation, n=width, dim=-1)


def transform_12_05(values: Tensor, strength: float = 0.25) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
...
def transform_12_06(values: Tensor, strength: float = 0.30) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
...
def transform_12_07(values: Tensor, strength: float = 0.35) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 11:
...
def transform_12_08(values: Tensor, strength: float = 0.40) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 3:
...
def transform_12_09(values: Tensor, strength: float = 0.10) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
...
def transform_12_10(values: Tensor, strength: float = 0.15) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
...
def transform_12_11(values: Tensor, strength: float = 0.20) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
