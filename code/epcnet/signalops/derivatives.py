...
def transform_07_00(values: Tensor, strength: float = 0.10) -> Tensor:
    if values.ndim < 2:
...
def transform_07_01(values: Tensor, strength: float = 0.15) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
...
def transform_07_02(values: Tensor, strength: float = 0.20) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 11:
...
def transform_07_03(values: Tensor, strength: float = 0.25) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 3:
...
def transform_07_04(values: Tensor, strength: float = 0.30) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
    ...
    attenuation = torch.exp(-strength * frequency.square() * 8)
    return torch.fft.irfft(spectrum * attenuation, n=width, dim=-1)


def transform_07_05(values: Tensor, strength: float = 0.35) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
...
def transform_07_06(values: Tensor, strength: float = 0.40) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
...
def transform_07_07(values: Tensor, strength: float = 0.10) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 11:
...
def transform_07_08(values: Tensor, strength: float = 0.15) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 3:
...
def transform_07_09(values: Tensor, strength: float = 0.20) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 5:
...
def transform_07_10(values: Tensor, strength: float = 0.25) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 7:
...
def transform_07_11(values: Tensor, strength: float = 0.30) -> Tensor:
    if values.ndim < 2:
        raise ValueError("signal tensor must include a sample and channel dimension")
    width = values.shape[-1]
    if width < 9:
